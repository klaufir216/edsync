...
type
  PuppyProgressCallback = proc (bytesComplete: int, bytesTotal: int)

type
  UnsafeClosure {.union.} = object
    ptrs: (pointer, pointer)
    closure: proc(bytesComplete: int, bytesTotal: int)

proc fetch*(req: Request, progressCallback: PuppyProgressCallback = nil): Response =
  ...
  when defined(windows) and not defined(puppyLibcurl):
    ...
    try:
      ...
      while true:
        ...
        if progressCallback != nil:
            progressCallback(i, result.headers["content-length"].parseInt)
    ...
    proc curlProgressFn(
        ...
        if clientp == nil:
            return
        var cb: ptr UnsafeClosure = cast[ptr UnsafeClosure](clientp)
        if cb != nil and cb[].closure != nil:
            cb.closure(int(dlnow), int(dltotal))
        return 0
    ...
    var unsafeClosureProgressCallback: UnsafeClosure
    var rawProcPtr: pointer = nil
    var rawEnvPtr: pointer = nil
    if progressCallback != nil:
        rawProcPtr = rawProc(progressCallback)
        rawEnvPtr = rawEnv(progressCallback)
        unsafeClosureProgressCallback = UnsafeClosure(ptrs: (rawProcPtr, rawEnvPtr))
        discard curl.easy_setopt(OPT_PROGRESSDATA, unsafeClosureProgressCallback.addr)
...
proc fetch*(url: string, headers = newSeq[Header](), progressCallback: PuppyProgressCallback = nil): string =
  let
    req = newRequest(url, "get", headers)
    res = req.fetch(progressCallback)
  if res.code == 200:
    return res.body
